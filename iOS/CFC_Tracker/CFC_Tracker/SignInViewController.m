...
- (void)gppInit {
  ...
  _sectionCellLabels = @[
    @[ kConnectToMovesLabel , kForceSyncLabel, kGetServerToken ]
  ];

  _switchCells = @[ ];
  _buttonCells = @[ kConnectToMovesLabel, kForceSyncLabel, kGetServerToken ];
...
- (IBAction)signIn:(id)sender {
    ...
    [_statsDb storeEventNow:@"button_account_changed"];
}

- (void)connectToMoves:(UIButton*)sender {
    // should launch the connect to moves here
    NSLog(@"Connecting to moves here!");
    [_statsDb storeEventNow:@"button_moves_linked"];
    
    UIApplication *ourApplication = [UIApplication sharedApplication];
    /**NSString *URLEncodedText = [@"app/authorize?client_id=<op9YOyXJ3PxwImHUTmO9msqKY8z4mmke>&redirect_uri=<http://54.218.218.130:1234/movesCallback>&scope=<location activity>" stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    NSString *ourPath = [@"moves://" stringByAppendingString:URLEncodedText];
    NSURL *ourURL = [NSURL URLWithString:ourPath];*/
    // NSURL *ourURL = [NSURL URLWithString:@"moves://app/authorize?client_id=<op9YOyXJ3PxwImHUTmO9msqKY8z4mmke>&redirect_uri=http://54.218.218.130:1234/movesCallback&scope=<location>"];
    // NSURL *ourURL = [NSURL URLWithString:@"moves://app/authorize?client_id=op9YOyXJ3PxwImHUTmO9msqKY8z4mmke&redirect_uri=https%3A%2F%2Fe-mission.shankari.org%2FmovesCallback&scope=location%20activity&state=11571871"];
    
    NSURL *ourURL = [[ConnectionSettings sharedInstance] getMovesURL];
    if ([ourApplication canOpenURL:ourURL]) {
        [ourApplication openURL:ourURL];
    }
    else {
        //Display error
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"Moves Not Found" message:@"Moves is not installed. It must be installed to connect." delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
        [alertView show];
    }
}
...
- (void)tableView:(UITableView *)tableView
  ...
  if ([label isEqualToString:kConnectToMovesLabel]) {
    [self connectToMoves:nil];
  } else if ([label isEqualToString:kForceSyncLabel]) {
      [_statsDb storeEventNow:@"button_sync_forced"];
